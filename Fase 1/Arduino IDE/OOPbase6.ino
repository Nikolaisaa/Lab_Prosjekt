...
public:
    ...
    int* getMotorSpeedsFromHighestIndex(int highest_sensor_value_index){
        int motorSpeeds[2] = {0,0};
        if(highest_sensor_value_index < 4){
        motorSpeeds[0] = 0;
        motorSpeeds[1] = 255;
        }
        else if(highest_sensor_value_index < 7){
            motorSpeeds[0] = 255;
            motorSpeeds[1] = 0;
        }
        else{
...
void loop() {
    ...
    //motors.setMotorSpeed(so.getMotorSpeedsFromHighestIndex(highestSensorValueIndex));
    delay(50);
}
